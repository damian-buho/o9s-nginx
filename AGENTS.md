...
└── includes/
    ├── http/     (24 files, numbered 010–240)
    ├── listen/   (6 files)
    ├── server/   (8 files)
    ├── opt/      (14 files)
...
- `includes/http/*.nginx.j2` — 24 numbered snippets injected via `include includes/http/*.nginx;` in the `http {}` block
...
`dhparams.pem` (2048-bit) generated at build time by `build.d/user/post/400-dhparams.sh` if not already present. 9 custom HTML error pages in `.container/user/app/errors/`.
