...
- Доступні перемикачі: CORS (`enable-cors`, 6 змінних), Content-Security-Policy (`enable-csp`, 20 змінних), HSTS (`enable-hsts`), Permissions-Policy (`enable-permissions-policy`, 12 змінних), сертифікат ACME (`enable-acme`), OTel на сервер (`enable-otel`), cache-control (`enable-cache`), favicon (`enable-favicon`), шляхи certbot (`enable-certbot`), X-Frame-Options DENY (`enable-sameorigin`), X-Content-Type-Options nosniff (`enable-nosniff`), перенаправлення HTTP на HTTPS (`redirect-to-https`).
